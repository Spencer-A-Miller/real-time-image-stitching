...
class MultiImageMatches:
    def __init__(self, images: list[Image], ratio: float = 0.75) -> None:
        ...
        """
        self.images = images
        self.matches = {i: {} for i in range(len(images))}
        self.ratio = ratio
    ...
    def get_matches(self, image_a: Image, image_b: Image) -> list:
        ...
        """
        index_a = self.images.index(image_a)
        index_b = self.images.index(image_b)

        if index_b not in self.matches[index_a]:
            matches = self.compute_matches(image_a, image_b)
            self.matches[index_a][index_b] = matches

        return self.matches[index_a][index_b]
